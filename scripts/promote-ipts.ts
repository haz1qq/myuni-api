/**
 * Promotes reviewed IPTS draft records into data/university/ and
 * data/campus/. The real Zod schemas (universitySchema / campusSchema) are
 * the gate -- a record is only written if the university AND every one of
 * its campuses pass validation as-is. Records that don't (usually missing
 * website, established, or student_range -- none of which are optional in
 * the schema) are left untouched in the draft file for a later enrichment
 * pass rather than promoted with a fabricated value that would crash
 * loadData() at server startup.
 *
 * Usage: npx tsx scripts/promote-ipts.ts [--category=university|university-college|college|all] [--dry-run]
 *
 * Idempotent: if data/university/<id>.json already exists, that record is
 * skipped entirely (not re-validated, not overwritten) -- once promoted, the
 * file in data/ is the source of truth and may have been hand-edited (e.g.
 * short_name cleaned up, a logo added), so re-running this script never
 * clobbers that.
 */
...
const ALL_CATEGORIES = ['university', 'university-college', 'college'];
...
function main(): void {
  ...
  for (const cat of categories) {
    const draftPath = path.join(OUT_DIR, `ipts-draft-${cat}.json`);
  ...
  if (skipped.length > 0) {
    const reportPath = path.join(OUT_DIR, 'ipts-promote-skipped.json');
